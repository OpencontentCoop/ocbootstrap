	...
	{else}
		<div class="maincontentheader">
			<form style="float: right" name="browse" method="post" action={$browse.from_page|ezurl}>
				<input class="defaultbutton" type="submit" name="BrowseCancelButton" value="{'Cancel'|i18n( 'design/admin/content/browse' )}" />
			</form>
	...
	<div class="clearfix">
	  <form action={"/content/search"|ezurl}>
		  <div class="input-group">
			  <input style="padding:6px" class="form-control" name="SearchText" type="text" value="{cond( ezhttp_hasvariable('SearchText','get'), ezhttp('SearchText','get'),'')}" size="12" />
			  <input type="hidden" value="Cerca" name="SearchButton" />
			  <input type="hidden" value="{$browse.start_node}" name="SubTreeArray[]" />
			  <input name="Mode" type="hidden" value="browse" />
			  <span class="input-group-btn">
				<button name="SearchButton" value="Cerca" id="searchbox_submit" type="submit" class="btn btn-info searchbutton">
	...
	<form class="clearfix" name="browse" method="post" action={$browse.from_page|ezurl}>
		...
		<div id="selection" class="context-block" style="margin: 20px 0">			
		  <div class="panel panel-success">
			<table id="select_table" class="table" cellspacing="0"><tbody></tbody></table>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var removeSelect = function(selection){
            ...
            $.each( cookies, function(i,v){ if (v==selection) cookies.splice(i, 1);} );
            eraseCookie('ocb_browse');
