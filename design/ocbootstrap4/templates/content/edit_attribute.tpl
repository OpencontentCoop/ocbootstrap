...
{if $count|gt(1)}
    ...
    <ul class="nav nav-tabs">
        ...
        {foreach $content_attributes_grouped_data_map as $attribute_group => $content_attributes_grouped}
            {if $attribute_group|ne('hidden')}
                <li class="nav-item">
                    <a class="nav-link{if $count|eq(0)} active{/if}" data-toggle="tab"
...
<div class="tab-content">
    {set $count = 0}
    {foreach $content_attributes_grouped_data_map as $attribute_group => $content_attributes_grouped}
        <div class="clearfix attribute-edit tab-pane{if $count|eq(0)} active{/if} p-2" id="attribute-group-{$attribute_group}">
            {set $count = $count|inc()}
            ...
            {foreach $content_attributes_grouped as $attribute_identifier => $attribute}
                ...
                <div class="row {$attribute_style} p-2 mb-4 ezcca-edit-datatype-{$attribute.data_type_string} ezcca-edit-{$attribute_identifier}">
