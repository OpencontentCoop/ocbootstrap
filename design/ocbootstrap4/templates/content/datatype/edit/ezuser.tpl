...
{else}
    <input autocomplete="off" placeholder="{'Username'|i18n( 'design/standard/content/datatype' )}"
           ...
           value="{if is_set($attribute.content.login)}{$attribute.content.login|wash()}{/if}"/>
{/if}
...
{if ezini( 'UserSettings', 'RequireConfirmEmail' )|eq( 'true' )}
    <p><input autocomplete="off" placeholder="{'Confirm email'|i18n( 'design/standard/content/datatype' )}"
              ...
              value="{cond( ezhttp_hasvariable( concat( $attribute_base, '_data_user_email_confirm_', $attribute.id ), 'post' ), ezhttp( concat( $attribute_base, '_data_user_email_confirm_', $attribute.id ), 'post')|wash( xhtml ), $attribute.content.email )}"/>
    </p>
{/if}
