...
<form action="{"/content/search/"|ezurl(no)}" method="get">
  ...
  <div class="input-group mb-2">
    <input type="text" name="SearchText" class="form-control form-control-lg" value="{$search_text|wash}" />
	...
	<div class="col-sm-3 hidden-xs">
                  ...
                  {foreach $facetData.nameList as $key2 => $facetName}
                      {if eq( $activeFacetParameters[concat( $defaultFacet['field'], ':', $defaultFacet['name'] )], $facetName )}
                          ...
                          {def $suffix=$uriSuffix
										  ...
										  |implode( '' )}
                          <li>
	                          <a class="badge badge-success" href={concat( $baseURI, $suffix )|ezurl} title="{'Remove filter on '|i18n( 'design/ezwebin/content/search' )}'{$facetName|trim('"')|wash}'">
          ...
          {if gt( $dateFilter, 0 )}
              <li>
                 ...
                 {def $suffix=$uriSuffix|explode( concat( '&dateFilter=', $dateFilter ) )|implode( '' )}
                  <a class="badge badge-success" href={concat( $baseURI, $suffix )|ezurl} title="{'Remove filter on '|i18n( 'design/ezwebin/content/search' )}'{$dateFilterLabel}'">
                      <span class="remover">&times</span>
                      ...
                      {foreach $facetData.nameList as $key2 => $facetName}
                          ...
                          <li>
                              <a href={concat(
                                  ...
                                  $uriSuffix )|ezurl}>
                                  <span class="badge badge-primary facet-name">{$facetName|shorten(20)|wash} <span class="badge badge-light">{$facetData.countList[$key2]}</span></span>
                              </a>
                          </li>
                          {/if}
                      {/foreach}
          ...
          {if eq( $dateFilter, 0 )}
              <li>
                  ...
                  <ul class="list-unstyled">
                    <li>
                        <a href={concat( $baseURI, '&dateFilter=1', $uriSuffix )|ezurl}><span class="badge badge-primary facet-name">{"Last day"|i18n("design/standard/content/search")}</span></a>
                    </li>
                    <li>
                        <a href={concat( $baseURI, '&dateFilter=2', $uriSuffix )|ezurl}><span class="badge badge-primary facet-name">{"Last week"|i18n("design/standard/content/search")}</span></a>
                    </li>
                    <li>
                        <a href={concat( $baseURI, '&dateFilter=3', $uriSuffix )|ezurl}><span class="badge badge-primary facet-name">{"Last month"|i18n("design/standard/content/search")}</span></a>
                    </li>
                    <li>
                        <a href={concat( $baseURI, '&dateFilter=4', $uriSuffix )|ezurl}><span class="badge badge-primary facet-name">{"Last three months"|i18n("design/standard/content/search")}</span></a>
                    </li>
                    <li>
                        <a href={concat( $baseURI, '&dateFilter=5', $uriSuffix )|ezurl}><span class="badge badge-primary facet-name">{"Last year"|i18n("design/standard/content/search")}</span></a>
                    </li>
                  </ul>
              </li>
           {/if}
