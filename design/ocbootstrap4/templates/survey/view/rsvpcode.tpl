<div class="form-group">
    <label for="{$prefix_attribute}_ezsurvey_answer_{$question.id}_{$attribute_id}">
      {$question.question_number}. {$question.text|wash('xhtml')} <strong class="required">*</strong>
    </label>
    ...
    <div class="survey-choices">
        ...
        {case match=1}
            ...
            {section-else}
                <input class="form-control border shadow" id="{$prefix_attribute}_ezsurvey_answer_{$question.id}_{$attribute_id}" name="{$prefix_attribute}_ezsurvey_answer_{$question.id}_{$attribute_id}"
                       type="text" size="{$question.num}"
                       value="{first_set( $#view_parameters.code, $question.answer )|wash('xhtml')}"/>
            {/section}
        {/case}
        {case}
            {section show=$question_result}
            {if is_set($survey_validation.post_variables.variables[$question.id])}
                {def $previous_vars=$survey_validation.post_variables.variables[$question.id]}
            {/if}
                <textarea class="form-control border shadow" id="{$prefix_attribute}_ezsurvey_answer_{$question.id}_{$attribute_id}" name="{$prefix_attribute}_ezsurvey_answer_{$question.id}_{$attribute_id}"
                          rows="{$question.num2}"
